/**
 * Email HTML rendering — pure functions with no side effects.
 *
 * Separated from email.ts so templates can be unit-tested and previewed
 * without SvelteKit env imports or network calls.
 *
 * VAP branding: Navy #1B365D, Sunflower #F2A906, Warm White #FFFBF1
 */

// ─── Layout Primitives ──────────────────────────────────────────────────────

/** Wraps email content in a branded layout shell (inline-styled for email clients). */
export function emailLayout(siteUrl: string, content: string, footerNote?: string): string {
    return `
<!DOCTYPE html>
<html lang="en">
<head>
...
/** Generates a sunflower-gold CTA button with navy text. */
...
/** Fallback URL block for copy-paste. */
...
/** Renders the email verification HTML. */
export function renderVerificationEmail(siteUrl: string, token: string): { subject: string; html: string } {
    ...
    return {
        ...
        html: emailLayout(siteUrl, content, 'This link expires in 24 hours. If you did not create a VASpeak account, you can safely ignore this email.'),
    };
}

/** Renders the magic link login HTML. */
export function renderMagicLinkEmail(siteUrl: string, token: string): { subject: string; html: string } {
    const loginUrl = `${siteUrl}/api/auth/magic-link/verify?token=${encodeURIComponent(token)}`;
    const content = `
    ...
    return {
        ...
        html: emailLayout(siteUrl, content, 'This link expires in 15 minutes and can only be used once. If you did not request this, ignore this email.'),
    };
}

/** Renders the password reset HTML. */
export function renderPasswordResetEmail(siteUrl: string, token: string): { subject: string; html: string } {
    ...
    return {
        ...
        html: emailLayout(siteUrl, content, 'This link expires in 1 hour. If you did not request a password reset, your account is safe — just ignore this email.'),
    };
}

/** Renders the newsletter welcome HTML. */
export function renderNewsletterWelcomeEmail(siteUrl: string, email: string): { subject: string; html: string } {
    ...
    return {
        ...
        html: emailLayout(siteUrl, content, `You are receiving this because you subscribed at VASpeak. <a href="${unsubscribeUrl}" style="color: #8A491B; text-decoration: underline;">Unsubscribe</a>`),
    };
}

/** Renders the unsubscribe confirmation HTML. */
export function renderUnsubscribeEmail(siteUrl: string, email: string): { subject: string; html: string } {
